...
x = SimpleRNN(32, activation='relu', return_sequences=True)(x)
x = Dropout(0.25)(x)
...
model_RNN = Model(inputs, outputs, name="RNN_model")
...
model_RNN.summary()
tf.keras.utils.plot_model(model_RNN, to_file='RNN_model.png')
...
model_RNN_history = model_RNN.fit(train_sentences,
                                  train_labels,
                                  epochs=200,
...
# EVALUATION RESULT
# Learning curves
learning_curves(model_RNN_history)

# Evaluation model
evaluation_model(model_RNN_history)

# Check the results
model_RNN.evaluate(val_sentences, val_labels)

# Predictions
model_RNN_pred_probs = model_RNN.predict(val_sentences)
model_RNN_preds = tf.squeeze(tf.round(model_RNN_pred_probs))
model_RNN_preds[:20]

model_RNN_results = calculate_results(y_true=val_labels,
                                      y_pred=model_RNN_preds)
model_RNN_results

# Compare model results
model_results = pd.DataFrame({"baseline": baseline_results,
                              "RNN_model": model_RNN_results})
