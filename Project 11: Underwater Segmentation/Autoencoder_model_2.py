...
# Conv 3
conv = Conv2D(filters=128, kernel_size=3, activation='relu', use_bias=True, padding="same")(conv)
conv = BatchNormalization()(conv)

# Conv 4
conv = Conv2D(filters=256, kernel_size=5, activation='relu', use_bias=True, padding="same")(conv)
conv = BatchNormalization()(conv)

# DECONVOLUTION
# Deconv 1
deconv = Conv2DTranspose(filters=128, kernel_size=2, activation='relu', padding="same")(conv)

# Deconv 2
deconv = Conv2DTranspose(filters=64, kernel_size=3, activation='relu', padding="same")(deconv)
...
model.compile(optimizer='Adam',
              ...
              metrics=["mse"])
model.summary()
tf.keras.utils.plot_model(model, to_file='Autoencoder_model_2.png')
