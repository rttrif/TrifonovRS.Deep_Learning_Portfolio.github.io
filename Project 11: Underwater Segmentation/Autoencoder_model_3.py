...
for image, mask in zip(train_df['image'], train_df['mask']):
    ...
    resized_x_image = cv2.resize(image_x, (128, 128))
    resized_x_mask = cv2.resize(mask_x, (128, 128))

    add_x = cv2.addWeighted(resized_x_image, 0.6, resized_x_mask, 0.6, 0.5)

    resized_x_add = cv2.resize(add_x, (128, 128))
...
# CONVOLUTION
# Conv 1
conv = Conv2D(filters=32, kernel_size=2, activation='relu', use_bias=True, padding="same")(input)
conv = BatchNormalization()(conv)

# Conv 2
conv = Conv2D(filters=64, kernel_size=3, activation='relu', use_bias=True, padding="same")(conv)
conv = BatchNormalization()(conv)
...
# Conv 4
conv = Conv2D(filters=128, kernel_size=5, activation='relu', use_bias=True, padding="same")(conv)
conv = BatchNormalization()(conv)

# DECONVOLUTION
# Deconv 1
deconv = Conv2DTranspose(filters=128, kernel_size=2, activation='relu', use_bias=True, padding="same")(conv)

# Deconv 2
deconv = Conv2DTranspose(filters=64, kernel_size=3, activation='relu', use_bias=True, padding="same")(deconv)

# Deconv 3
deconv = Conv2DTranspose(filters=64, kernel_size=3, activation='relu', use_bias=True, padding="same")(deconv)

# Deconv 4
deconv = Conv2DTranspose(filters=32, kernel_size=3, activation='relu', padding="same")(deconv)

# Output
output = Conv2DTranspose(filters=3, kernel_size=5, activation='sigmoid', padding="same")(deconv)
...
model.compile(optimizer='Adam',
              ...
              metrics=["mse"])
model.summary()
tf.keras.utils.plot_model(model, to_file='Autoencoder_model_3.png')
